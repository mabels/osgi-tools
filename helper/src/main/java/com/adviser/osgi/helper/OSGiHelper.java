...
public class OSGiHelper {
    ...
    public PackageAdmin getPackageAdmin() {
        ...
        if (pa == null) {
            LOG.error("No package admin available");
        }
        return pa;
    }
        ...
        if (refs.length != 0) {
            ...
            while (refs.length == 0) {
                ...
                if ((now - begin) > timeout) {
                    if (fail) {
                        throw new RuntimeException("Timeout ... no services matching with the request after " + timeout + " ms");
                    } else {
                        LOG.error("Timeout ... no services matching with the request after " + timeout +
                                " ms");
                        return null;
                    }
                }
    ...
    public Bundle installBundle(String url) {
        try {
            ...
            LOG.error("Cannot install the bundle " + url + " : " + e.getMessage());
        }
    ...
    public Bundle installAndStart(String url) {
        ...
        try {
            ...
            LOG.error("Cannot start the bundle " + url + " : " + e.getMessage());
        }
        return bundle;
    }
    ...
    public Bundle getBundle(String name) {
        ...
        LOG.error("No bundles with the given symbolic name " + name);
